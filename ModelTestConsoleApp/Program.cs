...
namespace ModelTestConsoleApp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            string connectionString = @"Data Source=.\SQLEXPRESS;Initial Catalog=ParkbeheerS;Integrated Security=True";
            ParkbeheerContext ctx = new(connectionString);
            ctx.Database.EnsureDeleted();
            ctx.Database.EnsureCreated();

            IHuizenRepository hrepo = new HuizenRepositoryEF(connectionString);
            BeheerHuizen bh = new(hrepo);
            ...
            BeheerHuurders bhuur = new(rhuur);
